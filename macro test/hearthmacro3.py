...
while run:
        ...
        if len(match_pts_cardall) != 0:
                ...
                pyautogui.click(match_pts_cardall[0, 0], match_pts_cardall[0, 1], button = "left") 
                pyautogui.dragTo(match_pts_cardall[0, 0], match_pts_cardall[0, 1]-450, button = "left") 
        
        #hero click
        sift2 = cv2.xfeatures2d.SIFT_create()        
        kp_hero1, des_hero1 = sift2.detectAndCompute(hero,None)
        kp_hero2, des_hero2 = sift2.detectAndCompute(wind,None)        
        bf2 = cv2.BFMatcher()
        matches2 = bf.knnMatch(des_hero1,des_hero2, k=2)
        match_pts_hero = []
        for m1, m2 in matches2:
